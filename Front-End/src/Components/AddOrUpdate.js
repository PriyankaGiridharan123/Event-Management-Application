  ...
  const {id} = useParams();
    const saveOrUpdate = (e) => 
    {
      ...
      if(age.length==0||name.length==0||phone.length==0||venue.length==0||event.length==0||noOfMembers==0){
        ...
        if (window.confirm("Confirm Your EventBooking!") == true) {
          e.preventDefault();
          ...
          if(id){
            EventService.updateEmployee(id, employee).then((response) => {
              ...
              EventService.createEmployee(employee).then((response) =>{
    ...
    <div className="container">
      ...
      <form>
        ...
        <div className="input">
          ...
          <option value="" disabled selected hidden>Choose Your Event</option>
